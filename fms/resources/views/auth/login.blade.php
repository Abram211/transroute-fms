...
<div style="width:100%;max-width:28rem">
    ...
    <div style="background:#fff;border-radius:.75rem;box-shadow:0 25px 50px -12px rgba(0,0,0,.5);overflow:hidden;padding:2rem">
        <h3 style="font-family:'Hanken Grotesk',sans-serif;font-size:1.5rem;font-weight:600;color:#00355f;margin-bottom:.5rem;text-align:center">Welcome Back</h3>
        <p style="font-size:.875rem;color:#42474f;margin-bottom:1.5rem;text-align:center">Enter your credentials to access your account</p>
        <form method="POST" action="{{ route('login') }}" style="display:flex;flex-direction:column;gap:1rem">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;font-size:.875rem">
                <label style="display:flex;align-items:center;gap:.5rem"><input type="checkbox" name="remember"> <span>Remember me</span></label>
            </div>
            <button type="submit" style="width:100%;background:#00355f;color:#fff;padding:1rem;border-radius:.5rem;font-weight:600;font-size:12px;letter-spacing:.05em;text-transform:uppercase;border:none;cursor:pointer">
                Login Now
            </button>
        </form>
        <p style="margin-top:1.5rem;font-size:.875rem;color:#42474f;text-align:center">Don't have an account? <a href="{{ route('register') }}" style="color:#00355f;font-weight:700;text-decoration:underline">Sign up</a></p>
        <p style="margin-top:.75rem;font-size:.75rem;color:#94a3b8;text-align:center"><a href="{{ route('admin.login') }}" style="color:#a04100">Administrator Login →</a></p>
    </div>
</div>
</body>
</html>
