...
<div class="bg-white rounded-xl border border-outline-variant/30 overflow-hidden">
    ...
    <table class="w-full text-left">
        ...
        <tbody class="font-data-tabular text-data-tabular">
            ...
            <tr class="zebra-row">
                <td class="p-sm border-b border-outline-variant/20 font-semibold">{{ $row['flight']->flight_number }}</td>
                <td class="p-sm border-b border-outline-variant/20">{{ $row['flight']->departureAirport->city }} → {{ $row['flight']->arrivalAirport->city }}</td>
                <td class="p-sm border-b border-outline-variant/20">{{ $row['passenger_count'] }}</td>
                <td class="p-sm border-b border-outline-variant/20">${{ number_format($row['ticket_revenue'], 2) }}</td>
                <td class="p-sm border-b border-outline-variant/20">{{ number_format($row['weight'], 1) }} kg</td>
                <td class="p-sm border-b border-outline-variant/20">${{ number_format($row['shipment_revenue'], 2) }}</td>
            </tr>
            @empty
            <tr><td colspan="6" class="p-md text-center text-on-surface-variant">No flight data yet</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
