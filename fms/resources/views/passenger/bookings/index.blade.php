    ...
    <div x-data="{ showBookModal: false }">
        <div class="flex items-center justify-between mb-md">
            <div>
                ...
                <p class="font-body-md text-body-md text-on-surface-variant">View, manage, and cancel your flight tickets.
                </p>
            </div>
            <button @click="showBookModal = true"
                class="bg-secondary-container text-on-secondary-container px-4 py-2 rounded-lg font-label-caps text-label-caps flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if ($ticket->isCancellable())
                            <form method="POST" action="{{ route('passenger.bookings.cancel', $ticket) }}"
                                ...
                                @csrf
                                <button type="submit"
                                    class="px-3 py-1 bg-red-50 text-error rounded-lg text-xs font-semibold hover:bg-red-100">Cancel</button>
                            </form>
                        @endif
                    </div>
                </div>
        ...
        {{-- Book Flight Modal --}}
        <div x-show="showBookModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            style="background:rgba(0,31,60,0.5);backdrop-filter:blur(4px);">
            <div class="bg-white rounded-xl max-w-lg w-full p-lg" @click.outside="showBookModal = false">
                ...
                <form method="POST" action="{{ route('passenger.bookings.store') }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="seat_no" placeholder="e.g. A5"
                            class="w-full px-3 py-2 border border-outline-variant rounded-lg text-sm">
                    </div>
                    <p class="text-xs text-on-surface-variant">Your booking will be submitted as "pending" until approved by
                        an administrator.</p>
                    <div class="flex gap-2 pt-2">
                        <button type="submit"
                            class="bg-primary text-on-primary px-4 py-2 rounded-lg font-label-caps text-label-caps">Submit
                            Booking</button>
