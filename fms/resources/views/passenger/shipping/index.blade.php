...
@section('content')
    <div>
        ...
        <p class="font-body-md text-body-md text-on-surface-variant mb-md">Shipments linked to your bookings are assigned by
            our team. This view is read-only.</p>
                ...
                <div
                    ...
                    <div class="flex items-center gap-base">
                        ...
                        <div>
                            ...
                            <div class="text-xs text-on-surface-variant">{{ $s->flight->flight_number }} ·
                                {{ $s->flight->departureAirport->city }} → {{ $s->flight->arrivalAirport->city }} ·
                                {{ $s->weight }} kg</div>
                        </div>
                    </div>
                    <div>
                        ...
                        <span
                            class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusColors[$s->status] ?? '' }}">{{ ucfirst(str_replace('_', ' ', $s->status)) }}</span>
                    </div>
                </div>
            @empty
