...
<div class="space-y-gutter">
    <div>
        ...
        <p class="font-body-md text-body-md text-on-surface-variant">Here's what's happening with your upcoming travel.</p>
    </div>
    ...
    <div class="space-y-base">
        ...
        <div class="bg-white rounded-xl border border-outline-variant/30 p-md flex flex-col md:flex-row md:items-center justify-between gap-base">
            <div class="flex items-center gap-base">
                ...
                <div>
                    <div class="font-semibold text-on-surface">{{ $ticket->flight->flight_number }} — {{ $ticket->flight->departureAirport->city }} → {{ $ticket->flight->arrivalAirport->city }}</div>
                    <div class="text-xs text-on-surface-variant">{{ $ticket->flight->departure_time->format('D, M d, Y · g:i A') }} · Gate {{ $ticket->flight->gate ?? 'TBA' }} · Seat {{ $ticket->seat_no ?? 'Unassigned' }}</div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                @php $statusColors = ['pending'=>'bg-yellow-100 text-yellow-700','confirmed'=>'bg-green-100 text-green-700']; @endphp
                <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusColors[$ticket->status] ?? '' }}">{{ ucfirst($ticket->status) }}</span>
            </div>
        </div>
        @empty
        <div class="bg-white rounded-xl border border-outline-variant/30 p-lg text-center text-on-surface-variant">
            ...
            No upcoming flights. <a href="{{ route('passenger.bookings.index') }}" class="text-secondary underline">Book one now</a>.
        </div>
        @endforelse
    </div>
    ...
    <div class="space-y-base">
        ...
        <div class="bg-white rounded-xl border border-outline-variant/30 divide-y divide-outline-variant/20">
            ...
            <div class="p-md flex items-start gap-3 {{ $note->is_read ? '' : 'bg-primary/5' }}">
                ...
                <span class="material-symbols-outlined text-primary mt-0.5">{{ $iconMap[$note->type] ?? 'notifications' }}</span>
                <div class="flex-1">
                    <div class="font-semibold text-sm text-on-surface">{{ $note->title }}</div>
                    <div class="text-xs text-on-surface-variant">{{ $note->message }}</div>
                    <div class="text-xs text-on-surface-variant/60 mt-1">{{ $note->sent_at?->diffForHumans() }}</div>
                </div>
                @if(!$note->is_read)
