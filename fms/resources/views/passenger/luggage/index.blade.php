...
@section('content')
    <div>
        ...
        <p class="font-body-md text-body-md text-on-surface-variant mb-md">Luggage is assigned by our team. You can confirm
            pickup once it has arrived.</p>
                ...
                <div
                    ...
                    <div class="flex items-center gap-base">
                        ...
                        <div>
                            ...
                            <div class="text-xs text-on-surface-variant">{{ $lug->ticket->flight->flight_number }} ·
                                {{ $lug->ticket->flight->departureAirport->city }} →
                                {{ $lug->ticket->flight->arrivalAirport->city }} · {{ $lug->weight }} kg</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusColors[$lug->status] ?? '' }}">{{ ucfirst(str_replace('_', ' ', $lug->status)) }}</span>
                        @if ($lug->status === 'in_transit')
                            <form method="POST" action="{{ route('passenger.luggage.status', $lug) }}">
                                ...
                                <button type="submit"
                                    class="px-3 py-1 bg-green-50 text-green-700 rounded-lg text-xs font-semibold hover:bg-green-100">Confirm
                                    Pickup</button>
                            </form>
                        @endif
                    </div>
